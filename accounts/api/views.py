...
async def send_verification_email(email, verification_url):
    ...
    body = (f'Iltimos, hisobingizni tasdiqlash uchun quyidagi havolaga kiring: {verification_url}\n'
            f'Eslatma: Havola 5 daqiqa davomida amal qiladi.')
    message.attach(MIMEText(body, "plain"))

    try:
        smtp = aiosmtplib.SMTP(hostname=settings.EMAIL_HOST, port=settings.EMAIL_PORT, use_tls=True)
        await smtp.connect()
